...
int affl_get_task(void)
{
	...
	for_each_process(task)
	{
		...
		else
		{
			...
			affl_link_name_path[0] = 0;
			printk("symbol_syze = %d\n", symbol_size);
			printk("affl_sys_readlink path = %s, link_name_path = %s\n", affl_path, affl_link_name_path);
			if (symbol_size < 0)
			{
			...
			else
			{
				...
				{
					strcpy(affl_link_name_path, strrchr(affl_link_name_path, '/'));
					affl_add_list_process_mass(affl_link_name_path + 1, task->pid);
				}
			}
		}
	}
